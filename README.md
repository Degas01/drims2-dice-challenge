...
| Task node logs `cannot look up dice_tf: "base_link" ... does not exist` and drops to `blind` | A TF listener only receives `/tf` while its node is being executed. | Fixed in `dice_task_node.main`: the node now runs in its own executor on a background thread, and `wait_for_tf` fails loudly instead of silently degrading. |
